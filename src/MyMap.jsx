 ...
 */
function createCentripetalCatmullRomSpline(points, segments = 20, alpha = 0.5) {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    for (let j = 1; j <= segments; j++) {
      ...
      const point = interpolateCatmullRom(p0, p1, p2, p3, scaledT, t0, t1, t2, t3);
      result.push(point);
    }
  }
...
function interpolateCatmullRom(p0, p1, p2, p3, t, t0, t1, t2, t3) {
  ...
  const m0 = {
    x: (t2 - t0) * ((p1.x - p0.x) / (t1 - t0) - (p2.x - p0.x) / (t2 - t0) + (p2.x - p1.x) / (t2 - t1)),
    y: (t2 - t0) * ((p1.y - p0.y) / (t1 - t0) - (p2.y - p0.y) / (t2 - t0) + (p2.y - p1.y) / (t2 - t1))
  };
  
  const m1 = {
    x: (t2 - t0) * ((p1.x - p0.x) / (t1 - t0) - (p3.x - p1.x) / (t3 - t1) + (p2.x - p1.x) / (t2 - t1)),
    y: (t2 - t0) * ((p1.y - p0.y) / (t1 - t0) - (p3.y - p1.y) / (t3 - t1) + (p2.y - p1.y) / (t2 - t1))
  };
...
function createSmoothQuadraticCurve(points, segments = 15, tension = 1) {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    for (let j = 0; j < numSubSegments; j++) {
      ...
      const segP0 = j === 0 ? p0 : {
        x: (1 - t1) * (1 - t1) * p0.x + 2 * (1 - t1) * t1 * cp1.x + t1 * t1 * p2.x,
        y: (1 - t1) * (1 - t1) * p0.y + 2 * (1 - t1) * t1 * cp1.y + t1 * t1 * p2.y
      };
      
      const segP2 = j === numSubSegments - 1 ? p2 : {
        x: (1 - t2) * (1 - t2) * p0.x + 2 * (1 - t2) * t2 * cp2.x + t2 * t2 * p2.x,
        y: (1 - t2) * (1 - t2) * p0.y + 2 * (1 - t2) * t2 * cp2.y + t2 * t2 * p2.y
      };
    ...
    if (distanceBetweenPoints > 0.5) {
      ...
      if (tempLine.length >= 3) {
        ...
        if (lastRadius < MIN_TURNING_RADIUS) {
          ...
          tempLineCartesian = createSmoothCurve(tempLineCartesian);
          // filter points that has Nan values
          tempLineCartesian = tempLineCartesian.filter((point) => !isNaN(point.x) && !isNaN(point.y));
          // convert back to latlng
          console.log("templine cartesien", tempLineCartesian);
          tempLine = tempLineCartesian.map((point) => cartesianToLatlng(point.x, point.y, 10));
          ...
          console.log("Adjusted Points:", tempLine);
